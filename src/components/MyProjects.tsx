import * as React from "react";

const MyProjects = () => {
  const allProjects = [
    {
      title: "Inspiration",
      logo: "https://inspiration.davidl.fr/social.png",
      what: "My mentor david ask me to rework the design of the site using TailwindCSS.",
      description:
        "This project is to learn TailwindCSS and work with it on other projects. With tatilwind we can update faster the CSS and we use a tool that is use by a lot of developers today",
      lien: "https://inspiration.davidl.fr/",
    },
    {
      title: "Art au Féminin",
      logo: "data:image/png;base64,",
      what: "Art au Féminin is a French podcast website about feminist artist.",
      description:
        "I rework the website to make it more user friendly and responsive, i change the design and migrate everything we use (plugins / extension) to be up to date.",
      lien: "https://www.artaufeminin.fr",
    },
    {
      title: "Matthys.dev",
      logo: "https://raw.githubusercontent.com/MatthysDucrocq/MatthysDev/main/src/images/grayLogo.png",
      what: "This website is my portofolio, you can see my projects and contact me.",
      description:
        "With this website, Recruiters can find my projects and contact me if they are interested by my profile.",
      lien: "https://matthys.dev",
    },
    {
      title: "BrawlApp",
      logo: "https://www.meme-arsenal.com/memes/9ed65d5cfbd6002def61d46b2dd2f64c.jpg",
      what: "A Flutter project to learn the langage Dart.",
      description:
        "We select BrawlStar because its a funny game and we want to learn Dart with a funny method. This is a school project.",
      lien: "https://github.com/MatthysDucrocq/projetflutter",
    },
    {
      title: "Symfony Project",
      logo: "https://avatars.githubusercontent.com/u/143937?v=4",
      what: "Symfony School Project to learn the langage.",
      description:
        "We made a E-shop project to learn symfony, i work with a friend for this school project and we use Composer and Github",
      lien: "https://github.com/Corentyn/ProjetSymfony",
    },
    {
      title: "Co'feature App",
      logo: "https://raw.githubusercontent.com/MatthysDucrocq/coffeatureApp/master/assets/img/astro.png",
      what: "Co'Feature App is a mobile application to find coffee shops in your area.",
      description:
        "In UX/UI courses i made mokeup and i find them pretty so i decide by myself to make this app during my free time.",
      lien: "https://github.com/MatthysDucrocq/coffeatureApp",
    },
    {
      title: "Disk Orelsan",
      logo: "https://pbs.twimg.com/profile_images/1461470035835301896/I_NT0dhM_400x400.jpg",
      what: "Three.js Project to make a 3D model of Orelsan disk.",
      description:
        "I made this during a night when i want to learn Three.js, Orelsan just release his disk so i decided to make a 3D model of it.",
      lien: "https://github.com/MatthysDucrocq/threeJsOrelsanDisk",
    },
    {
      title: "SpaceCraft",
      logo: "https://cdn0.iconfinder.com/data/icons/jumpicon-space-filled-line-1/32/-_Rocket-Space-Ship-Spacecraft-Launch-512.png",
      what: "React Native Project to make a space ship selection.",
      description:
        "I follow a React Native Bootcamp and i made this project during a 3 days course.",
      lien: "https://github.com/MatthysDucrocq/spacecraft",
    },
    {
      title: "Office Hours",
      logo: "https://i.pinimg.com/originals/8c/8d/2a/8c8d2a06f82a817f856b35ab1c6f6146.jpg",
      what: "Video Edit",
      description:
        "With my mentor David, we made every week a live where we discuss about the current computing tools we use to work. I edit the live and we upload videos on Youtube.",
      lien: "https://www.youtube.com/watch?v=cKEe5cArM_Y&list=PLmewDYeBL3XINJcq8pIsnM0XmDe3xyxdE",
    },
    {
      title: "Slides",
      logo: "https://images.assetsdelivery.com/compings_v2/muslumstock/muslumstock1811/muslumstock181100444.jpg",
      what: "My slides presentation.",
      description:
        "I made slides to share the tools that i like to use to other people.i made them by using mdx-deck who is to tools that you can use to upload your slides like a website.",
      lien: "https://matthys.dev/slides",
    },
  ];

  return (
    <div>
      <ul className="liste">
        {allProjects.map((project) => {
          return (
            <>
              <div className="flex mt-12 mb-12 m-auto ">
                <div className=" justify-center text-center w-1/3">
                  <a
                    href={project.lien}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="hover:text-purple-400"
                  >
                    <img
                      className="m-auto border-2 rounded-md border-purple-400 "
                      src={project.logo}
                      alt=""
                      width={128}
                      height={128}
                    />

                    <p className="p-4 ">{project.title}</p>
                  </a>
                </div>
                <div className="  w-2/3 text-left">
                  <div className="text-white">{project.what}</div>
                  <div className="text-white">{project.description}</div>
                </div>
              </div>
            </>
          );
        })}
      </ul>
    </div>
  );
};

export default MyProjects;
